...
def detect(save_img=True, out="static/images/test.jpg", source='data/samples/good_test.jpg'):
    ...
    colors = [[255, 245, 55], [170, 178, 32], [10,10,255]] # Color for binary classification in B,G,R
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in det:
                    ...
                    if save_img or view_img:  # Add bbox to image
                        if result is not None:
                            label = '%s %.2f' % (result, con)
                            print(label)
                            plot_one_box(xyxy, im0, label=label, color=colors[int(cls)])
                        else:
                            label = '%s %.2f' % (names[int(cls)], conf)
                            plot_one_box(xyxy, im0, label=label, color=colors[2])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default='images', help='source')  # input file/folder, 0 for webcam
